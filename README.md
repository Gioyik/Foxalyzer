...
Goal: provide developers with a browser-based toolset they can use to scan Firefox OS apps for security problems.

Base on the tool evaluation at https://wiki.mozilla.org/Security/B2G/JavaScript_code_analysis we chose JSPrime, scanjs and retire.js for the first run of tools. The concept of the add-on is to run each scanner on any relevant file within a Firefox OS app and to integrate the required controls and the tools' output into the app-manager/developer tools framework in a meaningful way.

We need to evaluate thwo promising concept:

1. Add a "Analyze" button to app-manager's app view (close to the debug button)
...
2. Make the tools output separately in an additional "Analysis" tab in the developer tools.
